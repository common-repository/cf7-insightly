...
* Author: CRM Perks
* Text Domain: contact-form-insightly
...
if( !class_exists( 'vxcf_insightly' ) ):

class vxcf_insightly {
 ...
 public  $url = "https://www.crmperks.com";
  ...
  public static $title='Contact Form Insightly Plugin';  
  public static $path = ''; 
  public static $slug = "";
...
$this->push($entry,$form,'',false);    
  ...
  */
  ...
  if($type == "admin"){
     if($ver<4.2){
  ?>
    <div class="error vx_notice notice" data-id="<?php echo esc_attr($id) ?>"><p style="display: table"><span style="display: table-cell; width: 98%"><span class="dashicons dashicons-megaphone"></span> <b><?php esc_html_e('Contact Form Insightly Plugin','contact-form-insightly-crm') ?>. </b><?php echo wp_kses_post($message);?> </span>
<span style="display: table-cell; padding-left: 10px; vertical-align: middle;"><a href="#" class="notice-dismiss" title="<?php esc_html_e('Dismiss Notice','contact-form-insightly-crm') ?>">dismiss</a></span> </p></div>
  <?php
     }else{
  ?>
  <div class="error vx_notice notice below-h2 is-dismissible" data-id="<?php echo esc_attr($id) ?>"><p><span class="dashicons dashicons-megaphone"></span> <b><?php esc_html_e('Contact Form Insightly Plugin','contact-form-insightly-crm') ?>. </b> <?php echo wp_kses_post($message);?> </p>
  </div>    
  <?php
     }
  }else{
...
*/
  ...
  $message = sprintf(__("%sContact Form Entries%s is required. %s it today!%s", 'contact-form-insightly-crm'), "<a href='https://www.crmperks.com/'>", "</a>", "<a href='https://www.crmperks.com/'>", "</a>");   
  }else if($installed === 2){ // not active
  $message = sprintf(__('Contact Form Entries is installed but not active. %sActivate Contact Form Entries%s to use the Contact Form Insightly Plugin','contact-form-insightly-crm'), '<strong><a href="'.wp_nonce_url(admin_url('plugins.php?action=activate&plugin=contact-form-entries-pro/contact-form-entries-pro.php'), 'activate-plugin_contact-form-entries-pro/contact-form-entries-pro.php').'">', '</a></strong>');  
  } else if($installed === 3){ // not supported
  $message = sprintf(__("A higher version of %sContact Form Entries%s is required. %sPurchase it today!%s", 'contact-form-insightly-crm'), "<a href='https://www.crmperks.com/'>", "</a>", "<a href='https://www.crmperks.com/'>", "</a>");
  }  
...
$value=trim(implode(' ',$value));
  ...
  */
  ...
  $link="N/A"; $desc=__("Added to ",'contact-form-insightly-crm');
  ...
  $desc.=$feed['object'];
  if($feed['status'] == 5){
   $desc=sprintf(__("%s Deleted",'contact-form-insightly-crm'),$feed['object']);   
  }
  }else if($feed['status'] == 4){
      $desc=sprintf(__("%s Filtered",'contact-form-insightly-crm'),$feed['object']); 
  }else{
  $desc= !empty($feed['error']) ? $feed['error'] : esc_html__("Unknown Error",'contact-form-insightly-crm');
  }
  $title=__("Failed",'contact-form-insightly-crm');   
  if($feed['status'] == 1){
  $title=__("Created",'contact-form-insightly-crm');   
  }else if($feed['status'] == 2){
  $title=__("Updated",'contact-form-insightly-crm');   
  }else if($feed['status'] == 4){
  $title=__("Filtered",'contact-form-insightly-crm');   
  }else if($feed['status'] == 5){
  $title=__("Deleted",'contact-form-insightly-crm');   
  }
  ...
  */
  ...
  printf('<input type="checkbox" name="'.esc_attr($this->id).'_update" id="'.esc_attr($this->id).'_update" value="1" /><label for="'.esc_attr($this->id).'_update" title="%s">%s</label><br /><br />', esc_html__('Create or update this entry in Insightly. The fields will be mapped according to the form feed settings.', 'contact-form-insightly-crm'), esc_html__('Send to Insightly', 'contact-form-insightly-crm'));
  } else {
...
if(!current_user_can($this->id."_send_to_crm")){return; }
  ...
  $button.= '<input type="submit" class="button button-large button-secondary alignright" name="'.esc_attr($this->id).'_send" style="margin-left:5px; '.$margin.'" title="'.__('Create or update this entry in Insightly. The fields will be mapped according to the form feed settings.','contact-form-insightly-crm').'" value="'.__('Send to Insightly', 'contact-form-insightly-crm').'" onclick="jQuery(\'#action\').val(\'send_to_crm\')" />';
  //logs button
    ...
    $button.= '<a class="button button-large button-secondary alignright" style="margin-left:5px; margin-top:5px; " title="'.__('Go to Insightly Logs','contact-form-insightly-crm').'" href="'.esc_url($log_url).'">'.__('Insightly Logs','contact-form-insightly-crm').'</a>';
...
do_action('plugin_status_'.$this->type,'activate');  
  ...
  * This string is sent to email and insightly
...
public static function clean($var){
  ...
  */
  ...
  $status=0;
  if(class_exists('vxcf_insightly_wp')){
      $status=1;
  }else if( file_exists(WP_PLUGIN_DIR.'/contact-form-insightly-crm/contact-form-insightly-crm.php')) {
  $status=2;
  } 
  return $status;
  }
 ...
 $offset = (int) get_option('gmt_offset');
  ...
  /**
  * Decrypts Values
  * @param array $info Insightly encrypted API info 
...
public function do_actions(){
  ...
  */
  ...
  $this->data=new vxcf_insightly_data();
  return $this->data;
  }
  ...
  $e=1000; $k=1;
    foreach($feeds as $feed){
          $data=isset($feed['data']) ? json_decode($feed['data'],true) : array(); 
           ...
           $feed['meta']=$meta;
           $feed['data']=$data;
 if(!empty($data['organization_check']) || !empty($data['opportunity_check']) || !empty($data['contact_check'])){
     $feeds_meta[$e++]=$feed; 
  }else{
  $feeds_meta[$k++]=$feed; 
  }
...
if(!empty($value)){
  ...
  if(isset($data['owner']) && $data['owner'] == "1"){
   $temp['OWNER_USER_ID']=array('value'=> $data['user'],'label'=>'OWNER_USER_ID');   
  }  
  if(!empty($data['assign_user']) && !empty($data['user_id']) ){
   $temp['RESPONSIBLE_USER_ID']=array('value'=> $data['user_id'],'label'=>'RESPONSIBLE_USER_ID');   
  } 
   if(!empty($data['assign_cat']) && !empty($data['cat']) ){
   $temp['CATEGORY_ID']=array('value'=> $data['cat'],'label'=>'CATEGORY_ID');   
  }
     if(!empty($data['assign_source']) && !empty($data['lead_source']) ){
   $temp['LEAD_SOURCE_ID']=array('value'=> $data['lead_source'],'label'=>'LEAD_SOURCE_ID');   
  }
     if(!empty($data['assign_status']) && !empty($data['lead_status']) ){
   $temp['LEAD_STATUS_ID']=array('value'=> $data['lead_status'],'label'=>'LEAD_STATUS_ID');   
  }
  

  //add account or contract

    if(!empty($data['organization_check']) && !empty($data['object_organization'])){
     $organization_feed=$data['object_organization']; 
       if( isset(self::$feeds_res[$organization_feed]) ){

   $organization_res=self::$feeds_res[$organization_feed];
  /////
  if(!empty($organization_res['id'])){
      $temp['Organisations']=array('value'=> $organization_res['id'],'label'=>'ORGANISATION_ID');    
  }else{ //if empty continue
      continue;
  }    
   }
    } 
        if(!empty($data['opportunity_check']) && !empty($data['object_opportunity'])){
     $opportunity_feed=$data['object_opportunity']; 
       if( isset(self::$feeds_res[$opportunity_feed]) ){

   $opportunity_res=self::$feeds_res[$opportunity_feed];
  /////
  if(!empty($opportunity_res['id'])){
      $temp['Opportunities']=array('value'=> $opportunity_res['id'],'label'=>'OPPORTUNITY_ID');    
  }else{ //if empty continue
      continue;
  }    
   }
  ...
  if(!empty($contact_res['id'])){
      $temp['Contacts']=array('value'=> $contact_res['id'],'label'=>'CONTACT_ID');    
  }else{ //if empty continue
      continue;
  }    
   }
...
$account=$this->post('account',$feed);
  ...
  if($this->post('id',$res)!=""){ 
      ...
      if($action == "Added"){
          if(empty($res['link'])){
  $msg=sprintf(__('Successfully Added to Insightly (%s) with ID # %s .', 'contact-form-insightly-crm'),$feed['object'],$res['id']);
          }else{
  $msg=sprintf(__('Successfully Added to Insightly (%s) with ID # %s . View entry at %s', 'contact-form-insightly-crm'),$feed['object'],$res['id'],$res['link']);
          }
  $screen_msg=__( 'Entry added in Insightly', 'contact-form-insightly-crm');
      }else{
            if(empty($res['link'])){
  $msg=sprintf(__('Successfully Updated to Insightly (%s) with ID # %s . View entry at %s', 'contact-form-insightly-crm'),$feed['object'],$res['id'],$res['link']);   
            }else{
  $msg=sprintf(__('Successfully Updated to Insightly (%s) with ID # %s .', 'contact-form-insightly-crm'),$feed['object'],$res['id']);   
            }
          if($event == 'delete'){  
     $screen_msg=__( 'Entry deleted from Insightly', 'contact-form-insightly-crm');
          }else{
     $screen_msg=__( 'Entry updated in Insightly', 'contact-form-insightly-crm');
          }
          }
   
  
  }else if($this->post('status',$res) == 4){
  $screen_msg=$msg=__( 'Entry filtered', 'contact-form-insightly-crm');    
  }else{
  $status=0; $screen_msg_class="error";
  $screen_msg=__('Errors when adding to Insightly. Entry not sent! Check the Entry Notes below for more details.' , 'contact-form-insightly-crm' );
  if($log_id!=""){
      //message for  bulk actions in logs
  $screen_msg=__('Errors when adding to Insightly. Entry not sent' , 'contact-form-insightly-crm' );    
  }
  $msg=sprintf(__('Error while creating %s', 'contact-form-insightly-crm'),$feed['object']);
  if($this->post('error',$res)!=""){
      $error= is_array($res['error']) ? json_encode($res['error']) : $res['error'];
  $msg.=" ($error)";
  
  $_REQUEST['VXGInsightlyError']=$msg; //front end form error for admin only
  }   
  if(!$is_admin){
      $info['msg']=$msg;
...
//  } 
  ...
  $log_link=' <a href="'.esc_url($log_url).'" class="vx_log_link" data-id="'.$insert_id.'">'.__('View Detail','contact-form-insightly-crm')."</a>";
 $screen_msg.=$log_link;
    }
    if($post_comment){
...
//$this->add_note($entry["id"], $msg);
  ...
  */
  ...
  $subject="Error While Posting to Insightly";
  ...
  $detail["Form Id"]=$form['id'];
    ...
    $email_info=array("msg"=>$info['msg'],"title"=>__('Insightly','contact-form-insightly-crm')." Error","info_title"=>"More Detail","info"=>$detail);
  ...
  /**
  * Get Objects from local options or from insightly
  *     
 ...
 if($refresh){
  ...
  if(!empty($option['objects']) && is_array($option['objects'])){
   $option['objects']=array_merge($objects,$option['objects']);   
  }
...
$vxcf_insightly=new vxcf_insightly();
$vxcf_insightly->instance();
$vx_cf['vxcf_insightly']='vxcf_insightly';
endif;
