...
.tablenav .tablenav-pages a,.tablenav-pages-navspan{display:inline-block;min-width:17px;border:1px solid #ccc;padding:3px 5px 7px;background:#e5e5e5;font-size:16px;line-height:1;font-weight:400;text-align:center}
  ...
  <div class="vx_wrap"> 
  <div>
  <h2  class="vx_img_head"><img alt="<?php esc_html_e("Insightly Feeds", 'contact-form-insightly-crm') ?>" title="<?php esc_html_e("Insightly Feeds", 'contact-form-insightly-crm') ?>" src="<?php echo $this->get_base_url()?>images/insightly-crm-logo.png?ver=1" /> <?php esc_html_e("Insightly Log", 'contact-form-insightly-crm'); ?>   <select style="min-width: 120px; max-width: 200px; margin-left: 12px; font-weight: normal;" id="vx_sel_feed">
  <option value=""><?php esc_html_e('All Feeds','contact-form-insightly-crm') ?></option>
  <?php   
...
<form id="vx_form" class="crm_form" method="get"><div>
  ...
  <input type="text" placeholder="<?php esc_html_e('Search','contact-form-insightly-crm') ?>" value="<?php echo esc_attr($this->post('search')) ?>" name="search" class="crm_input_inline">
  <?php
...
?>
  ...
  <option value=""><?php esc_html_e('All Objects','contact-form-insightly-crm') ?></option>
  <?php    
  ...
  <option value=""><?php esc_html_e('All Status','contact-form-insightly-crm') ?></option>
  <?php
  ...
  <option value=""><?php esc_html_e('All Times','contact-form-insightly-crm') ?></option>
  <?php
  ...
  <input type="text" name="start_date" placeholder="<?php esc_html_e('From Date','contact-form-insightly-crm') ?>" value="<?php if(isset($_REQUEST['start_date'])){echo esc_attr($_REQUEST['start_date']);}?>" class="vxc_date crm_input_inline" style="width: 100px">
  ...
  <button type="submit" class="button-secondary button crm_input_inline" title="<?php esc_html_e('Apply','contact-form-insightly-crm') ?>" id="vx_apply_bulk"><i class="fa fa-check"></i> <?php esc_html_e('Apply','contact-form-insightly-crm') ?></button>
  ...
  ?>
  <button type="button" name="tab_action" title="<?php esc_html_e('Export as CSV','contact-form-insightly-crm') ?>" id="vx_export" class="button-secondary button crm_input_inline vx_left_10"><i class="fa fa-download"></i> <?php esc_html_e('Export as CSV','contact-form-insightly-crm') ?></button> 
  <?php
  }
  ...
  if(isset($data['feeds'][0]['entry_id']) && $data['feeds'][0]['entry_id']!=""){
     $entry_id=$data['feeds'][0]['entry_id'];
      ?>
  <a href="<?php echo  esc_url( $log_link.'&entry_id='.$entry_id);?>" title="<?php echo sprintf(esc_html__('View Entry# %s Logs','contact-form-insightly-crm'),$entry_id); ?>" class="button vx_left_10"><i class="fa fa-hand-o-right"></i> <?php echo sprintf(esc_html__('View Entry# %s Logs','contact-form-insightly-crm'),$entry_id); ?></a><?php
  }}
 if($this->post('entry_id') !="" || $_log_id !=""){
          ?><a href="<?php echo esc_url($log_link);?>" title="<?php esc_html_e('View All Logs','contact-form-insightly-crm') ?>" class="button vx_left_10"><i class="fa fa-external-link"></i> <?php esc_html_e('View All Logs','contact-form-insightly-crm') ?></a>        
          <?php
      }
...
if($items>0){
  ?>
  <div class="tablenav-pages"> <span id="paging_header" class="displaying-num"><?php esc_html_e('Displaying','contact-form-insightly-crm') ?> <span id="paging_range_min_header"><?php echo esc_html($data['min']); ?></span> - <span id="paging_range_max_header"><?php echo esc_html($data['max']) ?></span> of <span id="paging_total_header"><?php echo esc_html($data['items']) ?></span></span><?php echo wp_kses_post($data['links']) ?></div>
 <?php
}
        ?>       
  </div>
  ...
  <th scope="col" class="manage-column vx_sort"  data-name="crm_id"><?php esc_html_e("Insightly ID", 'contact-form-insightly-crm') ?>
   <i class="fa fa-caret-<?php echo esc_attr($crm_order) ?> vx_sort_icon <?php echo esc_attr($crm_class) ?>"></i>                          
  </th>
  <th scope="col" class="manage-column vx_sort"  data-name="entry_id"><?php esc_html_e("Entry ID", 'contact-form-insightly-crm') ?>
  <i class="fa fa-caret-<?php echo esc_attr($entry_order) ?> vx_sort_icon <?php echo esc_attr($entry_class) ?>"></i>                   
  </th>
     <th scope="col"><?php esc_html_e('Feed ID', 'contact-form-insightly-crm') ?> </th> 
  <th scope="col" class="manage-column"  data-name="object"><?php esc_html_e("Description", 'contact-form-insightly-crm') ?>
  </th>
  <th scope="col" class="manage-column vx_sort"  data-name="time"><?php esc_html_e("Time", 'contact-form-insightly-crm') ?>
  <i class="fa fa-caret-<?php echo esc_attr($time_order) ?> vx_sort_icon <?php echo esc_attr($time_class) ?>"></i>
  </th>
  <th style="width: 40px"><?php esc_html_e('Detail','contact-form-insightly-crm') ?></th>
  </tr>
  </thead>
  ...
  <th><?php esc_html_e('Detail','contact-form-insightly-crm') ?></th>  
  </tr>
  ...
  <td class="vx_check_col"><input type="checkbox" name="log_id[]" value="<?php echo esc_attr($row['id']); ?>" class="crm_input_check"></td>
    <td class="vx_icon_col"><img src="<?php echo esc_url($base_url) ?>images/<?php echo esc_attr($row["status_img"]); ?>.png" alt="<?php echo $row["status"] ? esc_html__("Active", 'contact-form-insightly-crm') : esc_html__("Inactive", 'contact-form-insightly-crm');?>" title="<?php echo esc_html($row['title']);?>" class="crm_status_img" /></td>
...
$feed_link=$this->get_feed_link($row['feed_id'],$row['form_id']);
 ?><a href="<?php echo esc_url($feed_link) ?>" title="<?php esc_html_e('Edit Feed','contact-form-insightly-crm') ?>">#<?php echo $row['feed_id'] ?></a><?php  } ?>
 </td>
    ...
    <td><i class="vx_icons vx_detail fa fa-th-list" title="<?php esc_html_e('Expand Details','contact-form-insightly-crm') ?>"></i></td>  
  </tr>
  ...
  <tr>
    <td colspan="4" style="padding:20px;">
        <?php esc_html_e("No Record(s) Found", 'contact-form-insightly-crm'); ?>
    </td>
  </tr>
  <?php
  }
  ...
  ?>
    <div class="crm_actions tablenav">
   <a id="vx_clear_logs" class="button" title="<?php esc_html_e('Clear Insightly Log','contact-form-insightly-crm') ?>" href="<?php echo esc_url(wp_nonce_url(admin_url('admin.php?page='.$this->post('page')."&view=log&vx_tab_action_".$this->id."=clear_logs"),'vx_nonce','vx_nonce')); ?>"><?php esc_html_e('Clear Insightly Log','contact-form-insightly-crm') ?></a>
  <div class="tablenav-pages"> <span id="paging_header" class="displaying-num"><?php esc_html_e('Displaying','contact-form-insightly-crm') ?> <span id="paging_range_min_header"><?php echo esc_html($data['min']); ?></span> - <span id="paging_range_max_header"><?php echo esc_html($data['max']) ?></span> of <span id="paging_total_header"><?php echo esc_html($data['items']) ?></span></span><?php echo wp_kses_post($data['links']) ?></div>
    </div>
  <?php
  }
...
if(sel.val() == ""){
    alert('<?php esc_html_e('Please Select Action','contact-form-insightly-crm') ?>');
      return false;
}
if($(".crm_input_check:checked").length == 0){ 
    alert('<?php esc_html_e('Please select at least one entry','contact-form-insightly-crm') ?>');
    return false;
}
var action=sel.val();
if( $.inArray(action,["send_to_crm_bulk_force","send_to_crm_bulk"]) !=-1 && $(".crm_input_check:checked").length>4){
 if(!confirm('<?php esc_html_e('Exporting more than 4 entries may take too long.\\n Are you sure you want to continue?','contact-form-insightly-crm') ?>')){
  e.preventDefault();    
 }   
}
  })
   $("#vx_clear_logs").click(function(e){
      if(!confirm('<?php esc_html_e('Insightly Logs will be deleted permanently. Do you want to continue?','contact-form-insightly-crm') ?>')){
          e.preventDefault();
      }  
  })    
  ...
  $(document).on("click",".vx_detail,.vx_close_detail",function(e){
    ...
    if(td.is(":visible"))
    {
      next_tr.find('.vxa_entry').slideUp('fast',function(){
       ...
       if(is_main){
    icon.attr('title','<?php esc_html_e('Expand Detail','contact-form-insightly-crm'); ?>');    
    }        
     return;   
    }else{
     next_tr.show('fast'); 
    ...
    if(is_main){
    icon.attr('title','<?php esc_html_e('Collapse Detail','contact-form-insightly-crm'); ?>');    
    }    
    }
