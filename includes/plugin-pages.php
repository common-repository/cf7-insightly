...
if( !class_exists( 'vxcf_insightly_pages' ) ) {
...
* @since       1.0.0
*/
class vxcf_insightly_pages   extends vxcf_insightly{
    public $ajax=false;
...
public function add_bulk_send_crm_callcenter($list){ 
 $list['send_insightly']=__('Send to Insightly','contact-form-insightly-crm');
  return $list;  
}
public function bulk_send_crm_callcenter($msg,$action,$ids,$type){ 
    if($action == 'send_insightly'){
...
$entry['__vx_id']=$entry['id'];
        ...
        if(empty($notice)){
            $notice='Nothing Sent to Insightly';
        }
...
$push=$this->push($entry_detail,$form,'add_note');
  ...
  ?>
  <div class="vx_row">
  <label><input type="checkbox" name="<?php echo esc_attr($this->id) ?>_send" value="yes"> <?php esc_html_e('Send to Insightly','contact-form-insightly-crm') ?></label>
  </div>
  <?php  
}
    /**
  * Display custom notices
  * show insightly response
  * 
  */
   ...
   $slug=$this->get_slug();
      if ( $file == $slug ) {
          $settings_link=$this->link_to_settings();
            array_unshift( $links, '<a href="' .$settings_link. '">' . esc_html__('Settings', 'contact-form-insightly-crm') . '</a>' );
        }
        return $links;
   }
  /**
...
$page_title =__('Insightly for Contact Form 7','contact-form-insightly-crm');
$menu_title =__('Insightly','contact-form-insightly-crm');
...
$class='';
 if($res){
       $msg=__('Account Deleted Successfully','contact-form-insightly-crm');
  $class='updated';   
 }else{
       $msg=__('Error While Removing Account','contact-form-insightly-crm');
  $class='error';      
 }
...
die();
  ...
  $msg=__('You do not have permissions to export logs','contact-form-insightly-crm');
  ...
  $fields=array(); $field_titles=array("#",__('Status','contact-form-insightly-crm'),__('Insightly ID','contact-form-insightly-crm') ,__('Entry ID','contact-form-insightly-crm'),__('Description','contact-form-insightly-crm'),__('Time','contact-form-insightly-crm'));
  ...
  check_admin_referer('vx_nonce','vx_nonce');
  if(!current_user_can($this->id."_edit_settings")){ 
  $msg=__('You do not have permissions to clear logs','contact-form-insightly-crm');
  $this->display_msg('admin',$msg);
  return;   
  }
  $data=$this->get_data_object();
  $clear=$data->clear_logs();
       ...
       $msg=__('Error While Clearing Insightly Logs','contact-form-insightly-crm');
      $level="error";
      if(!empty($clear)){
      $msg=__('Insightly Logs Cleared Successfully','contact-form-insightly-crm');   
      $level="updated";
      }
  ...
  //
  self::$tooltips = array(
 'vx_feed_name' =>  esc_html__('Enter feed name of your choice.', 'contact-form-insightly-crm'),
  'vx_sel_object' => esc_html__('Select the Object to Create when a Form is Submitted.', 'contact-form-insightly-crm'),
   'vx_sel_account' =>__('Select the Insightly account you would like to export entries to.', 'contact-form-insightly-crm'),
  'vx_sel_form' => esc_html__('Select the Contact Form you would like to integrate with Insightly. Contacts generated by this form will be automatically added to your Insightly account.', 'contact-form-insightly-crm'),
  
  'vx_map_fields' => esc_html__('Associate your Insightly fields to the appropriate Contact Form fields.', 'contact-form-insightly-crm'),
  
  'vx_optin_condition' =>__('When the opt-in condition is enabled, form submissions will only be exported to Insightly when the condition is met. When disabled all form submissions will be exported.', 'contact-form-insightly-crm'),
  
  'vx_manual_export' => esc_html__('If you do not want all entries sent to Insightly, but only specific, approved entries, check this box. To manually send an entry to Insightly, go to Entries, choose the entry you would like to send to Insightly, and then click the "Send to Insightly" button.', 'contact-form-insightly-crm'),
  
    'vx_entry_notes' => esc_html__('Enable this option if you want to synchronize Contact Form entry notes to Insightly Object notes. For example , when you add a note to a Contact Form entry, it will be added to the Insightly Object selected in the feed.', 'contact-form-insightly-crm'),
    
      'vx_primary_key' => esc_html__('Which field should be used to update existing objects?', 'contact-form-insightly-crm'),
      
  
  'vx_disable_logs'=>__('When an order is sent to Insightly we store that order information in the database and show it in the Insightly Log. Check this box if you do not want to save the exported order information in the logs.','contact-form-insightly-crm'),
  
'vx_sources'=>__('Get Lead Sources list from insightly.','contact-form-insightly-crm'),
'vx_statuses'=>__('Get Lead Status list from insightly.','contact-form-insightly-crm'),
'vx_cats'=>__('Get Categories list from insightly.','contact-form-insightly-crm'),

'vx_source'=>__('Which Lead Source should be assigned to this object.','contact-form-insightly-crm'),
'vx_status'=>__('Which Lead Status should be assigned to this object.','contact-form-insightly-crm'),
'vx_cat'=>__('Which Category should be assigned to this object.','contact-form-insightly-crm'),

'vx_source_check'=>__('If enabled, a Lead Source will be assigned to the Object','contact-form-insightly-crm'),
'vx_status_check'=>__('If enabled, a Lead Status will be assigned to the Object','contact-form-insightly-crm'),
'vx_cat_check'=>__('If enabled, a Category will be assigned to the Object','contact-form-insightly-crm'),

'vx_assign_opportunity'=>__('Enable this option if you want to assign an opportunity this object.','contact-form-insightly-crm'),
'vx_sel_opportunity'=>__('Select Opportunity feed. Opportunity created by this feed will be assigned to this object.','contact-form-insightly-crm'),

'vx_assign_contact'=>__('Enable this option , if you want to assign a Contact to this object','contact-form-insightly-crm'),
'vx_sel_contact'=>__('Select Contact feed. Contact created by this feed will be assigned to this object','contact-form-insightly-crm'),

'vx_assign_organization'=>__('Enable this option , if you want to assign a Organization to this object','contact-form-insightly-crm'),
'vx_sel_organization'=>__('Select Organization feed. Organization created by this feed will be assigned to this object','contact-form-insightly-crm'),

   'vx_owner_check'=>__('Enable this option if you want to assign another object owner.','contact-form-insightly-crm'),
   'vx_owners'=>__('Get Users list from Insightly','contact-form-insightly-crm'),
   'vx_order_notes'=>__('Enable this option if you want to synchronize WooCommerce Order notes to Insightly Object notes. For example, when you add a note to a WooCommerce Order, it will be added to the Insightly Object selected in the feed.','contact-form-insightly-crm'),
   'vx_sel_owner'=>__('Select a user as a owner of this object','contact-form-insightly-crm'),
   
       'vx_entry_note'=>__('Check this option if you want to send more data as CRM entry note.', 'contact-form-insightly-crm'),
   'vx_note_fields'=>__('Select fields which you want to send as a note', 'contact-form-insightly-crm'),
   'vx_disable_note'=>__('Enable this option if you want to add note only for new CRM entry', 'contact-form-insightly-crm')
...
<div class="table_head_i"><?php esc_html_e('Capsule', 'contact-form-insightly-crm'); ?></div>  
<div class="vx_group">
...
echo $this->format_log_msg($log);
 ...
 $msg=__('You do not have permissions for this action','contact-form-insightly-crm');
...
if( $this->has_feed($form_id)) { 
 $boxes['insightly_crm']=array('title'=>'<i class="fa fa-plug"></i> '.__('Insightly', 'contact-form-insightly-crm'),'callback'=>array($this,'meta_box_html'));  
} return $boxes; 
}
...
public function has_feed($form_id) {
  ...
  */
  ...
  $action=$this->post('vx_action');
    $module='';
  if(strpos($action,'refresh_cats') === 0){
  $pos=explode('_',$action);
  $action='refresh_cats';
  $module=end($pos);    
  }
 ...
 if(!empty($info['meta']) ){
  ...
  switch($action){
        case"refresh_lead_sources":
    $sources=$api->get_lead_sources(); 

    $data=array();
    if(is_array($sources)){
    $res['status']="ok";
    $data['crm_sel_source']=$this->gen_select($sources,$status_sel,__('Select Lead Source','contact-form-insightly-crm'));
  ...
  $res['data']=$data;   
      break;  
      case"refresh_cats":
    $cats=$api->get_cats($module); 

    $data=array();
    if(is_array($cats)){
    $res['status']="ok";
    $data['crm_sel_cat']=$this->gen_select($cats,$status_sel,__('Select Category','contact-form-insightly-crm'));
      $meta[$module.'_cats']=$cats;   
    }else{
     $res['error']=$cats;   
    }
  ...
  $res['data']=$data;   
      break;  
          case"refresh_lead_statuses":
    $sources=$api->get_lead_status_list(); 

    $data=array();
    if(is_array($sources)){
    $res['status']="ok";
    $data['crm_sel_status']=$this->gen_select($sources,$status_sel,__('Select Lead Status','contact-form-insightly-crm'));
      $meta['statuses']=$sources;   
    }else{
     $res['error']=$sources;   
    }
  ...
  case"refresh_users":
    ...
    if(is_array($users)){
    $res['status']="ok";
    $data['crm_sel_user']=$this->gen_select($users,$owner_sel,__('Select User','contact-form-insightly-crm'));
...
if(isset($res['error'])){
    $res['status']='error';
    if(empty($res['error'])){
    $res['error']=__('Unknown Error','contact-form-insightly-crm');
    }
}
  die(json_encode($res));    
  }
  ...
  */
  public  function mapping_page(){
      ...
      wp_enqueue_style('vxc-tooltip');
         $tabs=array('feeds'=>__('Insightly Feeds','contact-form-insightly-crm') , 'logs'=>__('Insightly Log','contact-form-insightly-crm') , 'accounts'=>__('Insightly Accounts','contact-form-insightly-crm'), 'settings'=>__('Settings','contact-form-insightly-crm'));
  ...
  $(this).text('<?php esc_html_e('Show Key','contact-form-insightly-crm') ?>');  
  $(this).removeClass('vx_hidden');
  key.attr('type','password');  
  }else{
  $(this).text('<?php esc_html_e('Hide Key','contact-form-insightly-crm') ?>');  
...
start_tooltip();  
  ...
  */
  ...
  esc_html_e('You do not have permissions to access this page','contact-form-insightly-crm');    
  return;
  }
  ...
  <p>
  <?php esc_html_e("Feed deleted.", 'contact-form-insightly-crm') ?>
  </p>
  </div>
  <?php
  }
  ...
  <div class="updated fade" style="margin:10px 0;">
  <p>
  <?php esc_html_e("Feeds deleted.", 'contact-form-insightly-crm') ?>
  </p>
  </div>
  <?php
  }
...
$count=$this->data->delete_log($log_ids);
  $this->screen_msg(sprintf(__('Successfully Deleted %d Item(s)','contact-form-insightly-crm'),$count));  
    }
  ...
  if(!empty($entry) && is_array($entry)){ 
    ...
    $push=array('class'=>'error','msg'=>__('Entry Not Found','contact-form-insightly-crm'));  
  }
  ...
  case"time": $time_order=$order_icon; $time_class="";   break;    
    ...
    $bulk_actions=array(""=>__('Bulk Action','contact-form-insightly-crm'),"delete"=>__('Delete','contact-form-insightly-crm'),
  'send_to_crm_bulk'=>__('Send to Insightly','contact-form-insightly-crm'),'send_to_crm_bulk_force'=>__('Force Send to Insightly - Ignore filters','contact-form-insightly-crm'));
  $base_url=$this->get_base_url();
   $objects=$this->get_objects(); 
      $statuses=array("1"=>__("Created",'contact-form-insightly-crm'),"2"=>__("Updated",'contact-form-insightly-crm'),"error"=>__("Failed",'contact-form-insightly-crm'),"4"=>__("Filtered",'contact-form-insightly-crm'),"5"=>__("Deleted",'contact-form-insightly-crm')); 
...
//
  ...
  if(is_array($fields)){
        if(!self::$is_pro){
     $temp_fields=array(); $phones=array('PHONE','PHONE_HOME','PHONE_MOBILE','PHONE_OTHER','PHONE_ASSISTANT');
    foreach($fields as $k=>$v){
        if(empty($v['is_custom']) && !in_array($k,$phones)){ 
       $temp_fields[$k]=$v;     
        }
    }
   $fields= $temp_fields;
 ...
 $fields=$this->post('fields',$feed_meta); 
  ...
  foreach($fields as $k=>$v){
            $req=$this->post('req',$v);
          if( in_array($v['name'],array('contact_id','assigned_user_id','account_id','campaign_id','account_name'))){ $skipped_fields[$k]=$v; }
           else if($req == 'true'){
   $map_fields[$k]=$v;       
      } 
...
foreach($map as $field_k=>$field_v){
  ...
  $sel_fields=array(""=>__("Standard Field",'contact-form-insightly-crm'),"value"=>__("Custom Value",'contact-form-insightly-crm'));
  ...
  */
  ...
  public function set_logging_supported($plugins) {
      $slug=$this->plugin_dir_name(); 
        $plugins[$slug] = esc_html__('Insightly','contact-form-insightly-crm');
        return $plugins;
    }
  /**
...
$this->ajax=true;
  ...
  $msg=__("Please Choose Object",'contact-form-insightly-crm');
  }else  if(empty($_REQUEST['form_id'])){
  $msg=__("Please Choose Form",'contact-form-insightly-crm');
  }
  if($msg !=""){
  ?>
...
/*  $object=$this->post('object',$feed);
  ...
  $msg=__("Please Choose Account",'contact-form-insightly-crm');
  }else  if(empty($_REQUEST['form_id'])){
  $msg=__("Please Choose Form",'contact-form-insightly-crm');
  }
  if($msg !=""){
...
$field_options="<option>".esc_html__("Select Object",'contact-form-insightly-crm')."</option>"; 
  if(is_array($objects)){
...
die();
  ...
  */
  ...
  if(!current_user_can($this->id.'_read_settings')){
  $msg_text=__('You do not have permissions to access this page','contact-form-insightly-crm');   
  ...
  $this->uninstall();
  $uninstall_msg=sprintf(__("Contact Form Insightly Plugin has been successfully uninstalled. It can be re-activated from the %s plugins page %s.", 'contact-form-insightly-crm'),"<a href='plugins.php'>","</a>");
$this->screen_msg($uninstall_msg);
  ...
  $meta=isset($_POST['meta']) ? $this->post('meta') : array();
   ...
   $msgs['submit']=array('class'=>'updated','msg'=>__('Settings Changed Successfully','contact-form-insightly-crm'));
...
include_once(self::$path . "templates/settings.php");
 ...
 $offset=$this->time_offset();
  ...
  check_admin_referer("vx_nonce");
  if(!current_user_can($this->id."_edit_settings")){ 
  esc_html_e('You do not have permissions to save settings','contact-form-insightly-crm');
  return;   
  }
  $msgs['submit']=array('class'=>'updated','msg'=>__('Settings Changed Successfully','contact-form-insightly-crm'));
  ...
  if(!$valid_email){
      $msgs['submit']=array("class"=>"error","msg"=>__('Invalid Email(s)','contact-form-insightly-crm'));
  }
...
//  $force_check=true;
  ...
  if($con_class != "updated" ){
      $msg=__('Connection to Insightly is NOT Working','contact-form-insightly-crm');  
  }else{
     $msg=__('Connection to Insightly is Working','contact-form-insightly-crm');   
  }
  $title=__('Test Connection: ','contact-form-insightly-crm');
...
$feed= $this->data->get_feed($fid); 
  ...
  check_admin_referer("vx_nonce");
  if(!current_user_can($this->id.'_edit_feeds')){
  esc_html_e('You do not have permissions to edit/save feed','contact-form-insightly-crm'); 
  return;
  }
  //
...
if(is_array($feed_update) && is_array($feed)){
  ...
  if($is_valid){
      ...
      $msgs['save']=array('class'=>'updated','msg'=>sprintf(__("Feed Updated. %sback to list%s", 'contact-form-insightly-crm'), '<a href="'.$feed_link.'">', "</a>"));
  }
  else{
  $msgs['save']=array('class'=>'error','msg'=>__("Feed could not be updated. Please enter all required information below.", 'contact-form-insightly-crm'));
...
if(is_array($tags)){
  ...
  $fields['__vx_id']=array('id'=>'__vx_id','label'=>__('Entry ID','contact-form-insightly-crm')); 
$fields['_vx_form_id']=array('id'=>'_vx_form_id','label'=>__('Form ID','contact-form-insightly-crm')); 
$fields['_vx_form_name']=array('id'=>'_vx_form_title','label'=>__('Form Title','contact-form-insightly-crm')); 

  $this->fields=$fields=array('cf'=>array("title"=>__('Contact Form Fields','contact-form-insightly-crm'),"fields"=>$fields));
...
if(is_array($fields_arr['fields'])){
  ...
  */
  ...
  $modules=array(""=>__("Select Object",'contact-form-insightly-crm'));
 ...
 /* if(!$force_check && $api_check<$time){ //check validity period in settings tab
  ...
  $msg=__( 'Successfully Connected to Insightly','contact-form-insightly-crm' );
     if(isset($info['_time'])){
  ...
  $info['msg']='API Token is Not Valid'; 
  $info['class']="error"; 
  }
        }
    if($check){ 
