...
if( !class_exists( 'vx_crmperks_notice_vxcf_insightly' )):

class vx_crmperks_notice_vxcf_insightly extends vxcf_insightly{
    
public $plugin_url="https://www.crmperks.com";
public $review_link='https://wordpress.org/support/plugin/cf7-insightly/reviews/?filter=5#new-post';
...
$msg =sprintf(__('Want to save contact form  submissions? Manage contact form entries and and Resend any entry to CRM with free %sConatct Form Entries Plugin%s','crm-perks-forms'),'<a href="https://wordpress.org/plugins/contact-form-entries/" target="_blank">','</a>');
?>
...
public function review_notice() { 
 ...
 if(!empty($install_time) && is_array($install_time) && !empty($install_time['time']) && empty($install_time['review_closed'])){
  ...
  <p><?php echo sprintf(esc_html__( 'You\'ve been using Contact Form 7 insightly Plugin for some time now; we hope you love it!.%s If you do, please %s leave us a %s rating on WordPress.org%s to help us spread the word and boost our motivation.','contact-form-entries'),'<br/>','<a href="'.$this->review_link.'" target="_blank" class="vxcf_close_notice_a" rel="noopener noreferrer">','&#9733;&#9733;&#9733;&#9733;&#9733;','</a>'); ?></p>
...
<p><i class="fa fa-check"></i> insightly custom fields and phone number fields.</p>
...
<p><b><a href="https://wordpress.org/plugins/support-x/" target="_blank">Support X - Wordpress Helpdesk</a></b> Shows user tickets from HelpScout, insightly, FreshDesk, Desk.com and Teamwork in wordpress. Users can create new tickets and reply to old tickets from wordpress.</p>
<p><b><a href="https://www.crmperks.com/plugins/woocommerce-plugins/woocommerce-insightly-plugin/" target="_blank">WooCommerce Insightly Plugin</a></b> allows you to quickly integrate WooCommerce Orders with Insightly CRM.</p>
...
new vx_crmperks_notice_vxcf_insightly();
endif;
